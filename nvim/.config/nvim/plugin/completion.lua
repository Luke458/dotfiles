...
vim.pack.add({
  ...
  -- Copilot Chat
  { src = 'https://github.com/nvim-lua/plenary.nvim', checkout = 'master' },
  'https://github.com/CopilotC-Nvim/CopilotChat.nvim',
})

-- lazydev: Neovim Lua API types & completions for Lua config files
local ok_ld, ld = pcall(require, 'lazydev')
if ok_ld then
  ld.setup {
    library = {
...
local ok_cp, cp = pcall(require, 'copilot')
if ok_cp then
  cp.setup {
...
-- CopilotChat
local ok_cc, cc = pcall(require, 'CopilotChat')
if ok_cc then
  cc.setup {
    model = 'auto',
